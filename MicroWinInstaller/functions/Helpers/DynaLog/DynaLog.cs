...
namespace MicroWin.functions.Helpers.Loggers
{
    public class DynaLog
    {
        ...
        private static void RemoveLogArchives()
        {
            ...
            try
            {
                string[] LogArchives = Directory.GetFiles(Path.Combine(Environment.TempDir, "logs"), "MWInstaller_DynaLog_*.old", SearchOption.TopDirectoryOnly);
        ...
        public static void CheckLogAge()
        {
            ...
            if (File.Exists($"{Environment.TempDir}\\logs\\MWInstaller_DynaLog.log"))
            {
                logMessage("Log File Found. Checking log file creation DateTime...", false);
                try
                {
                    DateTime CreationDate = File.GetCreationTimeUtc($"{Environment.TempDir}\\logs\\MWInstaller_DynaLog.log");
                    if (CreationDate < DateTime.UtcNow.AddDays(DYNALOG_LOG_ARCHIVE_MINIMUM_THRESHOLD_DAYS))
                    {
                        ...
                        string ArchivedFileName = $"MWInstaller_DynaLog_{DateTime.UtcNow.ToString("yyMMdd-HHmm")}.old";
                        try
                        {
                            File.Move("{Environment.TempDir}\\logs\\MWInstaller_DynaLog.log", $"{Environment.TempDir}\\logs\\{ArchivedFileName}");
                            logMessage("The old log file has been archived. New messages will be shown in a new log file", false);      // A blank sheet of... logs?
                            File.SetCreationTimeUtc($"{Environment.Environment.TempDir}\\logs\\MWInstaller_DynaLog.log", DateTime.UtcNow);
                        }
                        catch (Exception ex)
                        {
        ...
        public static void logMessage(string message, bool getParentCaller = false)
        {
            ...
            try
            {
                if (!Directory.Exists(Path.Combine(Environment.TempDir, "logs")))
                {
                    Debug.WriteLine("Creating log directory...");
                    Directory.CreateDirectory(Path.Combine(Environment.TempDir, "logs"));
                }
                long fileLength = 0L;
                if (File.Exists(Path.Combine(Environment.TempDir, "logs", "MW_DynaLog.log")))
                {
                    fileLength = new FileInfo(Path.Combine(Environment.TempDir, "logs", "MW_DynaLog.log")).Length;
                }
                ...
                File.AppendAllText(Path.Combine(Environment.TempDir, "logs", "MWInstaller_DynaLog.log"), $"{(fileLength > 0 ? "\n" : "")}{messageLine}");
            }
            catch (Exception ex)
            {
